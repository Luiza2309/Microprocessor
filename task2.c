...
int main()
{
    ...
    {
        //parcurge numerele luate de la tastatura
        scanf("%hd", &nr_introdus);
        convertirea_unsigned_short(w, nr_introdus);
        for(int j=0; j<16; j+=Dim)
        {
            //parcurge secventele de lungime Dim ale numerelor citite
            int putere=0;
            for(int k=j+Dim-1; k>=j; k--)
            {
                //parcurge bitii din fiecare secventa si transforma in baza 10
                operanzi[m]+=w[k]*pow(2, putere);
                putere++;
            }
            m++;
        }
